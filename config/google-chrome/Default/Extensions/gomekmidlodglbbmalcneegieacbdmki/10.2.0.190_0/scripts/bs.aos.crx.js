...
(function(_) {
  ...
  function applyRules (tabId, url) {
    ...
    // no rule?? stop here
    if(!rule || !/^http(s):\/\/?/gi.test(url)) return;
...
(function(AvastWRC, chrome, _) {
  ...
  var EDITIONS_CONFIG = [
    // AOS
    { extType: AvastWRC.EXT_TYPE_AOS,  callerId: 1016, reqUrlInfoServices: 0x000F, extVer: 15, dataVer: 15 }, // AOS (WR, P, B, SC)
    // ABOS
    { extType: AvastWRC.EXT_TYPE_ABOS, callerId: 7016, reqUrlInfoServices: 0x000F, extVer: 15, dataVer: 15 } // AOS (WR, P, B, SC)
  ];
