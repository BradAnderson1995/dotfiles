...
(function(_, PROTO) {
    ...
    /**
     * Log super function. Use log(a,b,c,d) to log your results. We will create an empty function for build, so it does not flood the console.
     * @return {[type]} [description]
     */
    function log(){
        // if(AvastWRC.DEFAULTS.LOG) console.log.apply(console, arguments);
    }
 ...
 ******************************************************************************/
 ...
 _.extend(AvastWRC,{
        ...
        getUrlInfo : function(urls, callback, notvisited){
            ...
            for(var i=0, j=urls.length; i<j; i++) {
                ...
                if(cUrl && AvastWRC.bs.checkUrl(cUrl)) {
                    ...
                    log("Ignoring url: " , cUrl);
                }
            }

            //console.log("I have to download ", download.length, " from total ", urls.length);
    ...
    AvastWRC.bs = {
         ...
         * It can handle remote files from http, https, ftp (assuming we are getting the full url), Browsers should override this to be able to handle also local file
         * E.G. for Chrome see loadFile implementation in background.js that replaces this method
    ...
    AvastWRC.Cache = {
        ...
        /**
         * Get chached record -> goes through .ttl() to check expiration
        ...
        get : function(key, multirequest) {
            log("Cache get", key, this.cache[key]);
            if(this.cache[key]) {
