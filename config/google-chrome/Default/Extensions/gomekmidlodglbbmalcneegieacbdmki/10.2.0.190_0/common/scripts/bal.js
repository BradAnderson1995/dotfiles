...
(function(_, EventEmitter) {
  ...
  var _injectLibs = {
    ...
    libs: [
      ...
      'common/libs/jquery.mustache.js',
  ...
  AvastWRC.bal = {
    ...
    httpGet: function(options) {
      options.type = 'GET';
      AvastWRC.bal.httpRequest(options);
    },
    /**
     * http POST helper
     * @param  {Object}
    ...
    /**
     * implicit http request response handler
     * @param  {Object}   xhr
     * @param  {Object}   data
     ...
     * @return {void}
     *
     * TODO - handle specific error codes. Can be useful for OAuth
     */
    httpSuccessResponseHandler: function(xhr, data, callback, errorHandler) {
      if(xhr.status < 400) {
        callback(data);
      } else {
        (errorHandler ? errorHandler : sing.defaultErrorHandler)(data, xhr);
      }
    },
    /**
     ...
     * @return {void}
     *
     * TODO - handle server not responding/offline
     */
    httpRequest: function(options) {
      var xhr = new XMLHttpRequest();
      xhr.open(options.type || "GET", options.url, options.async != undefined ? options.async : true);
      if(options.mimeType){
        for(i=0; i < options.mimeType.length; i++){
          xhr.setRequestHeader(options.mimeType[i].header, options.mimeType[i].value);
        }
      }
      if(options.responseType) {
        xhr.responseType = options.responseType;
      }
      xhr.onload = function(){ sing.httpSuccessResponseHandler(xhr, xhr.response, options.callback, options.errorHandler); }
      xhr.onerror = function(){ (options.errorHandler ? options.errorHandler : sing.defaultErrorHandler)(this.response, this); };
    ...
    getHostFromUrl: function(url){
      if(url == undefined ||
        ...
        url.toLowerCase().indexOf('chrome') == 0 ||
        url.toLowerCase() == 'about:newtab' ||
...
(function(AvastWRC, _) {
  ...
  AvastWRC.bal.DNT = AvastWRC.bal.DNT || {
     ...
     */
    isTracking: function(url, host, tabId) {
      ...
      if (urlHost && urlHost !== host) { // do not check resources from same host
        ...
        if (tracker) {
          ...
          AvastWRC.bal.emitEvent('badgeInfoUpdated', tabId, host, sing.DNT.computeTotal.bind(this));
      ...
      if(dnt && host){
        ...
        for(var i = sing.DNT.trkrType.length -1; i>=0; i--)
          ret.elements.push( {
            ...
            swichers  : [AvastWRC.bs.getLocalImageURL('switcher_dotred.png'), AvastWRC.bs.getLocalImageURL('switcher_dotgreen.png'), AvastWRC.bs.getLocalImageURL('switcher_dotorange.png')],
        ...
        for(var i=0 ;i < ret.elements.length; i++){
          ...
          item.swicher = item.swichers[item.state];
