...
(function($, EventEmitter) {
  ...
  AvastWRC.ial = AvastWRC.ial || {
    ...
    createMessageBox: function(data) {
      ...
      if($('#avast-inpage').length <= 0) {
        ...
        AvastWRC.ial.getTopHtmlElement()
          .prepend($($.mustache(data.isOldGui ? AvastWRC.Templates.messageBoxOld : AvastWRC.Templates.messageBox, data)));
    ...
    /**
     * Monitors snsitive fields for Credit Card warning functionality
     * @return {[type]}
     */
    ...
    onBlurHandler: function() {
      ...
      if(AvastWRC.ial.checkInput($el)) {
        ...
        $("#avast-title").remove();
        $(AvastWRC.ial._isOldGui ? "#avast-body P" : "#avast-body").html(AvastWRC.ial._messageBoxData.strings.inputwarning);
    ...
    showAlertBox: function(message){
      var alertBox = $('<div/>',{/*id: Math.random(),*/ class: 'avast_os_ext_alertBox' })
                      .click(function(){ $(this).fadeOut(500,AvastWRC.ial.selfRemove); })
                      .html(message);
      ...
      $('.avast_os_ext_alertBox').each(function(index){
        $(this).css({'z-index': '2147483646'});
        $(this).animate({'top': parseInt($(this).css('top')) + parseInt($(this).css('height')) + 60 + 'px'});
      });
  ...
  AvastWRC.ial.registerEvents(function(ee) {
    ee.on('message.initEnvironment', AvastWRC.ial.initEnvironment);
    ee.on('message.reInit',          AvastWRC.ial.reInitPage);
    ee.on('message.showAlertBox',    AvastWRC.ial.showAlertBox);
  ...
  AvastWRC.ial.aos = {
    ...
    cascadeBigSwitches: function(type) {
      var count = 0 ,switches = $('#avast_os_ext_rightContent .avast_os_ext_dnt_companies')
                        .find('.avast_os_ext_dnt_company_icon[data-type=' + type + ']'),
          bigSwitch = $('#avast_os_ext_rightContent .avast_os_ext_dnt_group_swicher[data-type=' + type +']');
    ...
    changeBigSwitch: function(element, state, cascade) {
      var newPos = state == 0 ?'left':'right', css, trk_ids = [],
          el = element.find('.avast_os_ext_dnt_group_swicher'),
      ...
      cache.swicher = cache.swichers[state];
      ...
      css['background-image'] = 'url(' + cache.swichers[state] + ')';
      el.css(css);
    ...
    fillDNT: function() {
      $(SEL_SIDEBAR_MIDDLE)
        .html($($.mustache(AvastWRC.Templates.dnt, AvastWRC.ial.aos._data.dnt)));
      AvastWRC.ial.aos._page = 1;

      $('#avast_os_ext_rightContent')
        .find('.avast_os_ext_dnt_group_right_wrapper')
          .click(function(element){
            var swicher = $(element.currentTarget).find('.avast_os_ext_dnt_group_swicher');
            AvastWRC.ial.aos.changeBigSwitch($(element.currentTarget), swicher.css('left') !== '-1px'? 0 : 1, true);
          });
    ...
    fillDetails: function() {
      if(AvastWRC.ial.aos._data.details){
        $(SEL_SIDEBAR_MIDDLE)
          .html($($.mustache(AvastWRC.Templates.details, AvastWRC.ial.aos._data.details)));
    ...
    fillThanks: function(rating, flags) {
      $(SEL_SIDEBAR_MIDDLE)
        .html($($.mustache(AvastWRC.Templates.thanks, AvastWRC.ial.aos._data.thanks)));
    ...
    fillRatingDetails: function(data) {
      if(data){
        $(SEL_SIDEBAR_MIDDLE)
          .html($($.mustache(AvastWRC.Templates.ratingExtra, data)));
    ...
    fillRating: function() {
      $(SEL_SIDEBAR_MIDDLE)
        .html($($.mustache(AvastWRC.Templates.rating, AvastWRC.ial.aos._data.rating)));
    ...
    fillContent: function(response){
      response.webrep.logo = AvastWRC.ial.bs.getLocalImageURL('logo_avast.png');
      $('#avast_os_ext_rightContent').html($($.mustache(AvastWRC.Templates.header, response.webrep || { nodata: true })));
    ...
    createTopBar: function(data) {
      if($('.avast-extension-topbar').length > 0) {
        return;
      }
      ...
      bar = AvastWRC.ial.topBar(
        $($.mustache(AvastWRC.Templates.topBar, data)),
        '.avast-extension-topbar',
        '50px'
      );

      $('.avast-extension-topbar .avast-extension-topbar-button').click(
        function() {
          proceed(true, false);
        });

      $('.avast-extension-topbar .avast-extension-topbar-decline-button').click(
        function() {
          proceed(false, true);
          bar.remove();
        });

      $('.avast-extension-topbar .avast-extension-topbar-close').click(
        function() {
          proceed(false, false);
          bar.remove();
        });
