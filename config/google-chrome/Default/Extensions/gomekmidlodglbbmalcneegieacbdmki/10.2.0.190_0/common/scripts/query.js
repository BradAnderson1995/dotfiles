(function(_, AvastWRC, PROTO) {
  ...
  AvastWRC.Query.__MASTER__ = {
          ...
          if(p.values_[prop].length) {
            ...
            for(var krop in p.values_[prop].values_) {
              if(p.values_[prop].values_[krop] instanceof PROTO.I64) {
                ...
                //console.log("converting", p.values_[prop].values_[krop])
              }else {
    ...
    identity : function() {
      ...
      msg = this.setBaseIdentityIds(msg);

      msg.browserType = AvastWRC.gpb.All.BrowserType[browserInfo.getBrowser()];

      msg.browserVersion = browserInfo.getBrowserVersion();
      //consoel.log("verison", AvastWRC.CONFIG.VERSION)
      //msg.pluginVersion =  PROTO.encodeUTF8(AvastWRC.CONFIG.VERSION);
      //msg.auid = PROTO.encodeUTF8("YYY");
      if(AvastWRC.DEFAULTS.USER) {
        //msg.uuid
        //msg.token
      }
     ...
     */
    extIdentity : function() {
      var msg = this.identity();
      return this.setExtIdentityIds(msg);
    },
    /**
  ...
  AvastWRC.Query.Avast = function(options){
      ...
      if(!options.type) {
        //console.log("ERROR! Trying to communicate with program with unknown command", options, options.type);
        return;
      }
  ...
  AvastWRC.Query.Avast.prototype = _.extend({},AvastWRC.Query.__MASTER__,{
    message : function(type){
      ...
      this.request.browser = 3;// AvastWRC.gpb.All.BrowserType[AvastWRC.Utils.Browser.get("browser")];
      ...
      switch(this.options.type){
        ...
        case "GET_PROPERTIES":
          for(var i=0, j=this.options.params.length; i<j; i++){
      ...
      //console.log("Avast Message", this.request)
      return this;
    }
  });
  ...
  AvastWRC.Query.UrlInfo = function(options) {
      ...
      this.options = _.extend({
        ...
        server : AvastWRC.Query.CONST.SERVER + ":" + AvastWRC.Query.CONST.PORT + "/" +
          (AvastWRC.CONFIG.FEATURES.newUrlInfoVersion ? AvastWRC.Query.CONST.URLINFO_V4 : AvastWRC.Query.CONST.URLINFO),
        method : "post",
        webrep : true,
        phishing : true,
        blocker : false,
  ...
  AvastWRC.Query.UrlInfo.prototype = _.extend({},AvastWRC.Query.__MASTER__,{
    ...
    message : function() {
      ...
      if (AvastWRC.CONFIG.FEATURES.newUrlInfoVersion) {
        this.request.client = this.clientIdentity();
      } else {
        this.request.identity = this.extIdentity();
      }
      ...
      if(AvastWRC.CONFIG.COMMUNITY_IQ === false) {
  ...
  AvastWRC.Query.Vote.prototype = _.extend({},AvastWRC.Query.__MASTER__,{
    message : function(){
      ...
      if(this.options.method.toLowerCase() == "put") {
        ...
        //console.log(this.request.vote.flags)
      } else {
