...
(function($, global) {
    ...
    var tag = "JqueryMutationSummary",
        ...
        JqueryMutationSummary = function(element, options) {
            this.$element = pns$(element);
            this.options = pns$.extend(true, {}, this.internalDefaults, pns$.fn.mutationSummary.defaults, options);
        },
    ...
    pns$.extend(true, privateFunctions, {
        getConfiguration: function(callback, observeOwnChanges, queries) {
            var configuration;

            if (pns$.isFunction(callback)) {
                if (pns$.isArray(observeOwnChanges)) {
    ...
    JqueryMutationSummary.prototype = {
        ...
        disconnect: function(callback, observeOwnChanges, queries) {
            ...
            pns$.each(summaries, function(index) {
    ...
    JqueryMutationSummaryInner.prototype = {
        ...
        getCallbackWrapper: function() {
            function callbackWrapper(summaries) {
                ...
                summaries.configuration = pns$.extend(true, {}, this.configuration);

                this.originalCallback(summaries);
            };

            return pns$.proxy(callbackWrapper, this);
        }
        ...
        start: function() {
            ...
            this.wrappedConfiguration = pns$.extend(true, {}, this.configuration);
    ...
    // Add jQuery method
    // pns$("#element").mutationSummary();
    // pns$("#element").mutationSummary("method", arguments);
    pns$.fn.extend({
    ...
    pns$.fn.mutationSummary.defaults = {};

    pns$.fn.mutationSummary.Constructor = JqueryMutationSummary;
}(jQuery, this));