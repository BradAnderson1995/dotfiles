...
var megalogo='';
if (!m) megalogo = '<img alt="Mega" src="' + staticpath + 'images/mega/blogs/jobs-mega-logo_v2.png"  class="blog-jobs-mega" />';
...
function unsigned_blogposts(ready)
{
	...
	if (typeof loadingDialog == "object") {
		loadingDialog.show()
		$('.blog-new-full,.blog-new-right').hide();
	}
	xhr.onreadystatechange = function() {
		if (this.readyState == 4) {
			...
			blogposts = []
			for (var i in tmp) {
				i = blogposts.push(tmp[i]) - 1
				for (var x in blogposts[i]['attaches']) {
					blogposts[i][x] = "https://cms.mega.nz/unsigned/" + blogposts[i]['attaches'][x]
				}
				blogposts[i]['by'] = blogposts[i]['by'] || "Admin"
			}
			if (typeof loadingDialog == "object") {
				loadingDialog.hide()
				$('.blog-new-full,.blog-new-right').show();
			}
			ready();
		}
	};
...
function init_blog()
{
	if (!blogposts) return unsigned_blogposts(init_blog)
	...
	{		
		...
		{		
			...
			if (a >= blogstart && a < bloglimit*blogpage)
			{
				...
				blogcontent +='<div class="clear"></div><img alt="" src="' + blogposts[i].simg + '" />';
...
if (typeof mobileblog !== 'undefined')
{
	...
	for (var i in blogposts)
	{
		...
		{
			var content = '';
			if (blogposts[i].bimg) content += '<img alt="" src="' +  blogposts[i].bimg + '" class="blog-new-full-img" />';
