...
function accountUI()
{
	...
	M.accountData(function(account)
	{
		...
		if (u_attr.p)
		{
			...
			if (account.stype == 'S')
			{
				...
				api_req({a:'ufp7s',c:0},
				{
					...
					{
						// if > 0 then show cancel btn and bind cancellation API call to the button
						if (res > 0)
						{
							$('.cancelbtn').show();
							...
							{
								msgDialog('confirmation', l[6822], l[6823],false,function(e)
								{
									if(e) 
									{
										$('.cancelbtn').hide();
										...
										api_req({a:'ufp7s',c:1},
										{
											callback: function()
											{
												// reset account cache and refetch all account data to display UI (note potential race condition if cancellation callback wasn't received in 7500ms)
		...
		$('.fm-account-save').bind('click',function(e)
		{
			...
			else if ($('#account-confirm-password').val() !== '' && $('#account-password').val() !== '' && $('#account-confirm-password').val() !== $('#account-password').val())
			{
				loadingDialog.show();
		...
		if (M.account.reseller)
		{
			...
			var oldVoucherType = $('.fm-account-select.vouchertype select').val();
			...
			var voucheroptions = '';
			for (var i in prices) {
				var price = htmlentities(prices[i]);
				var sel = oldVoucherType && oldVoucherType == price ? ' selected="selected"':'';
				voucheroptions += '<option value="'+ price +'"'+ sel +'>&euro;' +price + ' voucher</option>';
			}
		...
		if (is_chrome_firefox || ssl_needed()) $('#acc_use_ssl').hide();
