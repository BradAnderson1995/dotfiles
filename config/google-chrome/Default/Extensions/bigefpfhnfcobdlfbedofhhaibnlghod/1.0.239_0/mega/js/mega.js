...
//		$.transferHeader();
    ...
    /**
     * Transfer to mega.nz
     * @param {String} toPage The page e.g. 'pro', 'fm/account' etc
     */
    this.transferToMegaNz = function(toPage)
    {
        // If logged in, transfer the session (anything after the # is kept local)
        if (u_k && u_sid && toPage && u_privk) {
            var data = btoa(JSON.stringify([u_k, u_sid, toPage, u_privk]));
...
function execsc(ap, callback)
{
	...
	function done() {
		if (async_treestate.length) {
			async_treestate.forEach(function(a) {
				var n = M.d[a.n];
				if (n)
				{
