...
var backgroundPage = (function () {
    ...
    function setSignupABTest() 
    {
        if(!getSetting("signupSameTab") || getSetting("signupSameTab") !== 'ext_simplesignup_control')
        {
            setSetting("signupSameTab","ext_simplesignup_control");
            // var rand = Math.floor(Math.random()*4+1);
            // if (rand == 1 || (navigator.language !== 'en-US' && navigator.language !== 'en'))
            // {
            //     setSetting("signupSameTab","ext_simplesignup_control");
            // }
            // else if (rand == 2)
            // {
            //     setSetting("signupSameTab","ext_simplesignup_simpletag1");
            // }
            // else if (rand == 3)
            // {
            //     setSetting("signupSameTab","ext_simplesignup_simpletag2");
            // }
            // else if (rand == 4)
            // {
            //     setSetting("signupSameTab","ext_simplesignup_simpletag3");
            // }
        }
    }
    ...
    function loadSignupUIIntoPage(tab, callback) {
        var self = this;
        ...
        function injectCSSJS() {
            ...
            if (!isSafari()) {
                executeStyleFromURLInTab(tab, 'sites/global/base.css');
                executeStyleFromURLInTab(tab, 'sites/global/proximanova_chrome.css');
                executeStyleFromURLInTab(tab, 'sites/global/ext_signup.css');
                executeStyleFromURLInTab(tab, 'sites/global/ext_signup_chrome.css');
                executeScriptFromURLInTab(tab, 'sites/global/jquery-2.1.1.min.js');
                executeScriptFromURLInTab(tab, 'sites/global/jquery.tokeninput.min.js');
            }
            else {
                executeStyleFromURLInTab(tab, safari.extension.baseURI + 'sites/global/base.css');
                executeStyleFromURLInTab(tab, safari.extension.baseURI + 'sites/global/proximanova.css');
                executeStyleFromURLInTab(tab, safari.extension.baseURI + 'sites/global/ext_signup.css');
                executeScriptFromURLInTab(tab, "sites/global/jquery-2.1.1.min.js", function() {});
                executeScriptFromURLInTab(tab, "sites/global/jquery.tokeninput.min.js", function() {});
            }
        }
        ...
        if (isSafari()) {
            ...
            executeScriptInTabWithCallback(tab, 'window.___PKT__INJECTEDSIGNUP;', function(results) { 
                if (!results || typeof results[0] == 'object')
                {
                    injectCSSJS();                    
                }
        ...
        ril.sendAnalyticsCall(getSetting('guid'),'loading_simplepanel');
        executeScriptInTab(tab, "window.___PKT__SIGNUP_VARIANT = '" + getSetting("signupSameTab") + "'; window.___PKT__SIGNUP_TAGLINE = '" + tagline + "'; window.___PKT__INJECTEDSIGNUP = true;");
        executeScriptFromURLInTabWithCallback(tab, pkt.i18n.getFilePathForPocketOverlayLocalization(), function() 
        {
            executeScriptFromURLInTabWithCallback(tab, 'js/inject-signup.js', callback);
        });
    }
    ...
    addMessageListener(function messageListenerCallback(request, sender, sendResponse) {
        ...
        else if (request.action === "saveCurrentTab") 
        {
            getCurrentTab(function(tab) 
            {
                if (typeof tab === "undefined" || typeof tab.url === "undefined" || tab.url === "chrome://newtab/") 
                {
                    return;
                }
                var linkInfo = 
    ...
    (function initialize() {
        ...
        // randomly set A/B test for signupsametab
        setSignupABTest();

        // set popup logic
        if (isChromeOnly() && (getSetting("signupSameTab") == 'ext_simplesignup_simpletag1' || getSetting("signupSameTab") == 'ext_simplesignup_simpletag2' || getSetting("signupSameTab") == 'ext_simplesignup_simpletag3'))
        {
            if (!ril.isAuthorized())
            {
                chrome.browserAction.setPopup({popup:'html/signup.html?variant=' + getSetting('signupSameTab')});
            }
            else
            {
        ...
        if (!boolFromString(getSetting("installed"))) 
        {
            ...
            if (getSetting("signupSameTab") == 'ext_simplesignup_control')
            {
